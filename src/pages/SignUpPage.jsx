...
export function SignUpPage() {
  ...
  const { signUp, signInWithGoogle, loading, user } = useAuthStore();
  ...
  const handleNext = (e) => {
    ...
    if (step === 2 && (!formData.email || !formData.phone)) {
      setErrorMsg('Please enter your email and phone.');
      return;
    }
    setErrorMsg('');
    setStep((prev) => prev + 1);
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      ...
      if (data?.user && !data?.session) {
        ...
        setStep(1); // Reset wizard state visually or hide form
      } else {
        navigate('/dashboard');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#f2d4a8] flex items-center justify-center p-4 sm:p-6 pt-24 relative overflow-hidden">
      ...
      >
        {step > 1 && (
          <button onClick={handlePrev} className="absolute left-6 top-8 text-brand-brown/50 hover:text-brand-brown">
            <FiArrowLeft size={24} />
          </button>
        )}
        ...
        <div className="mb-8 flex gap-2 justify-center">
          {[1, 2, 3].map((i) => (
            <div key={i} className={`h-1.5 w-12 rounded-full ${step >= i ? 'bg-brand-green' : 'bg-black/10'}`} />
          ))}
        </div>
        ...
        {successMsg && (
          <div className="mb-4 rounded-xl bg-green-50 p-4 text-sm text-brand-green border border-green-100 font-bold text-center">
            {successMsg}
          </div>
        )}

        <form onSubmit={step === 3 ? handleSignUp : handleNext} className="space-y-4">
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="step1" variants={formVariants} initial="initial" animate="animate" exit="exit" className="space-y-4">
            ...
            {step === 2 && (
              <motion.div key="step2" variants={formVariants} initial="initial" animate="animate" exit="exit" className="space-y-4">
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-brown/40">
                    <FiMail size={20} />
                  </div>
            ...
            {step === 3 && (
              <motion.div key="step3" variants={formVariants} initial="initial" animate="animate" exit="exit" className="space-y-4">
                <div className="relative">
                  <div className="absolute left-4 top-1/2 -translate-y-1/2 text-brand-brown/40">
                    <FiLock size={20} />
                  </div>
          ...
          <div className="pt-4">
            <button
              type="submit"
              disabled={loading}
              className="group relative w-full overflow-hidden rounded-[1.25rem] bg-brand-green py-4 font-body text-[1.1rem] font-bold text-white flex items-center justify-center gap-2 shadow-[0_8px_20px_rgba(95,166,59,0.25)] transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_12px_28px_rgba(95,166,59,0.35)] active:scale-[0.98] disabled:opacity-70 disabled:hover:scale-100"
            >
              ...
              <span className="relative">{step < 3 ? 'Continue' : loading ? 'Creating Account...' : 'Finish Signup'}</span>
              {step < 3 && <FiArrowRight className="relative" />}
            </button>
          </div>
        </form>
